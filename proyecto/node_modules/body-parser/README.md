...
middlewares will populate the `req.fields` property with the parsed body, or an
...
object after the middleware (i.e. `req.fields`).
...
A new `body` object containing the parsed data is populated on the `request`
object after the middleware (i.e. `req.fields`). This will be a `Buffer` object
of the body.
...
object after the middleware (i.e. `req.fields`). This will be a string of the
body.
...
A new `body` object containing the parsed data is populated on the `request`
object after the middleware (i.e. `req.fields`). This object will contain
...
app.use(function (req, res) {
  ...
  res.end(JSON.stringify(req.fields, null, 2))
})
```
...
app.post('/login', urlencodedParser, function (req, res) {
  ...
  res.send('welcome, ' + req.fields.username)
})

// POST /api/users gets JSON bodies
app.post('/api/users', jsonParser, function (req, res) {
  if (!req.fields) return res.sendStatus(400)
  // create user in req.fields
})
```
